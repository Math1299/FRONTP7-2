            ...
            <v-container>
                ...
                <div v-if="showPosts">
                    <v-card class="forum__post ma-3 mt-6" v-for="(post, index) in allPosts" v-bind:key="index">
                        ...
                        <v-card-subtitle class=" forum__post__name"> Par {{ post.firstName }} {{ post.lastName }}, le {{ post.date }} </v-card-subtitle>
...
export default {
    ...
    methods: {
        clickPosts() {
            (this.showPosts = true), (this.showComments = false);
        },
        clickComments() {
            (this.showPosts = false), (this.showComments = true);
        },
        deletePost(pId) {
            axios
                .delete("http://localhost:5000/api/moderation/post/" + pId, { headers: { Authorization: "Bearer " + localStorage.token } })
                .then((response) => {
        ...
        deleteComment(cId) {
            axios
                .delete("http://localhost:5000/api/moderation/comment/" + cId, { headers: { Authorization: "Bearer " + localStorage.token } })
                .then((response) => {
                    let rep = JSON.parse(response.data);
                    console.log(rep.message);
                    window.location.assign("http://localhost:8080/Accueil/Moderation");
    ...
    mounted() {
        axios
            .get("http://localhost:5000/api/moderation/posts", { headers: { Authorization: "Bearer " + localStorage.token } })
            .then((response) => {
        ...
        axios
            .get("http://localhost:5000/api/moderation/comments", { headers: { Authorization: "Bearer " + localStorage.token } })
            .then((response) => {
